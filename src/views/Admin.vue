        ...
        <el-tab-pane label="座位管理" name="seats" v-if="selectedScreening">
          ...
          <div class="seat-map">
            <div v-for="row in seatRows" :key="row" class="seat-row">
              <el-button v-for="col in seatCols" :key="col" :type="seatType(seatMatrix[row]?.[col] || 0)"
                @click="toggleSeat(row, col)" :disabled="!seatMatrix[row]">
                {{ row }}-{{ col }}
              </el-button>
            </div>
          </div>
          ...
          <el-table :data="orders" style="margin-top:10px;">
            ...
            <el-table-column label="操作" width="260">
              <template #default="scope">
                ...
                <el-button size="small" type="success" :disabled="scope.row.status === 'PAID'"
